...
describe( 'listener-registry-local-load-balancing', function() {
	...
	describe( 'with a single provider', function(){
		...
		it( 'accepts a subscription for 2 clients and does nothing when provider unsubscribes', function() {
			// 1. provider does listen a/.*
			tu.providerListensTo( 1, 'a/.*' )
			// 2.  client 1 requests a/1
			tu.clientSubscribesTo( 1, 'a/1' )
	...
	describe( 'with multiple providers', function(){
		...
		it( 'first rejects, seconds accepts, third does nothing', function() {
...
describe( 'listener-registry-local-load-balancing does not send publishing updates for events', function() {
	beforeEach(function() {
		tu = new ListenerTestUtils( 'E' );
	});

	it( 'client with provider already registered', function() {
		// 1. provider does listen a/.*
		tu.providerListensTo( 1, 'a/.*' )
		// 2.  client 1 requests a/1
		tu.clientSubscribesTo( 1, 'a/1' )
